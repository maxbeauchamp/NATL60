...
if __name__ == '__main__':
    ...
    for i in range(0,len(daterange)):
        ...
        nadir=nadir.convert_on_grid(basepath+'/src/subgrid1_natl60.txt',basepath+'/src/mask_subgrid1_natl60.txt')
        # modifications of time values to force unique time values after regridding
        new_time = [(np.datetime64(datetime.strptime(date,'%Y-%m-%d') + timedelta(seconds=dt))-\
                     np.datetime64('2012-10-01T00:00:00Z')) / np.timedelta64(1, 's') \
        ...
        swot=swot.convert_on_grid(basepath+'/src/subgrid1_natl60.txt',basepath+'/src/mask_subgrid1_natl60.txt')
        # modifications of time values to force unique time values after regridding
        new_time = [(np.datetime64(datetime.strptime(date,'%Y-%m-%d') + timedelta(seconds=dt))-\
                     np.datetime64('2012-10-01T00:00:00Z')) / np.timedelta64(1, 's') \
        ...
        nadir_swot=nadir_swot.convert_on_grid(basepath+'/src/subgrid1_natl60.txt',basepath+'/src/mask_subgrid1_natl60.txt')
