...
def plot_data_and_maps(date,nadir_lag,diro,extent):
    ...
    nadir.plot("ssh_mod",diro+"/"+date+"/NATL60_nadir_"+date+"_nadlag"+str(nadir_lag)+"d.png")
    # convert swot to daily swot files
    swot=NATL60_swot.init2(date,date)
    swot.set_extent(extent)
    swot.plot("ssh_mod",diro+"/"+date+"/NATL60_swot_"+date+".png")
    # nadir-swot fusion 
    nadir_swot_fusion=NATL60_fusion(nadir,swot)
    nadir_swot_fusion.plot("ssh_mod",diro+"/"+date+"/NATL60_nadir_swot_"+date+"_nadlag"+str(nadir_lag)+"d.png")
...
if __name__ == '__main__':
    ...
    extent=[-65.,-55.,30.,40.]
    #extent=[-80.,0.,25.,65.]
    plot_data_and_maps(date,1,datapath+"/plots",extent)
